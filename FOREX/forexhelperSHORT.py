...
def calculate_investment(entry_price, stop_loss_price, initial_capital, risk_levels, lot_price, pip_value, spread):
    ...
    for risk in risk_levels:
        ...
        def calculate_loss_for_lots(lots):
            ...
            loss_per_lot = (stop_loss_price - entry_price) * adjusted_pip_value  # Loss per lot if the price goes against
            spread_loss_entry = spread * lots  # Spread cost at entry
            spread_loss_exit = (spread * lots) * (entry_price / stop_loss_price)  # Spread cost if closing at stop loss level
...
def calculate_take_profit(entry_price, highest_point, lowest_point):
    ...
    h = highest_point - lowest_point  # Calculate the distance between the highest and lowest points
    take_profit_price = entry_price - (2 / 3) * h  # Take profit price for SHORT: lower than entry
