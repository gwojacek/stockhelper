...
- download/cache daily market data from Stooq, Yahoo Finance, and Stooq web/table fallbacks, with targeted Stooq-tail repair when Yahoo-precision candles contaminate forex or commodity caches;
- scan market groups for Ichimoku cloud, Fibonacci setups, and falling-wedge (Kliny) formations with age-aware breakout/retest handling, a five-day invalid-save warning lifecycle, wick-based stop validation, anchor-aware monthly-range rejection, saved manual wedges, and alternate-wedge review;
- generate compact Trójpolówki (3P) Fibo/Ichimoku watchlists plus dedicated Kliny and persistent Favorites tabs from the latest allsearch data, with bilingual searchable/filterable HTML tabs, persistent instrument colors, checked-instrument lists, PNG table export, and grouped quick-chart buttons;
- open an interactive chart tool, search and switch between locally cached instruments, use a shared saved PLN balance, select price levels, measure percentage differences, inspect/import/edit wedge lines, save/update config files, export chart snapshots, and create transaction journal entries;
...
| Build combined report | `stock -allsearch all` | Runs scanners, refreshes latest candles, creates combined Markdown/HTML reports, and auto-opens the local HTML report URL. A market may continue with a final warning when no more than 10% of its instruments remain stale after repair. |
...
- **Ichimoku cloud scanner** for WIG, DAX/DAX40, Nasdaq-100/US100, ETFs, forex, commodities, or a single instrument, with local-extreme retest validation, early-breakout probation, breakout/retest metadata forwarded into chart Setup information, and direction-aware breakout highlights anchored to the first close beyond the cloud.
- **Fibonacci formation scanner** with long setups across supported markets, regular mirrored-short search for forex, commodities, DAX40, and US100, and directional short 3P impulse discovery for stocks; movable strong-impulse/23.6/61.8 lifecycle states; one-, two-, and three-candle reversal windows in which any pattern candle may touch 61.8; confirming-candle pattern dates; wick-based post-pattern stop invalidation; true-extreme and structural re-anchoring; anchor-aware completed month-range rejection, including volatile oscillating shelves; saved chart-anchor overrides that receive a visible five-day invalid warning before automatic removal; and an explain/debug mode.
...
- **Falling-wedge (Kliny) scanner** exported from the Fibo scan flow, including unbroken wedges and fresh breakouts (up to 5 candles after breakout), automatic structures capped at 30 candles, Avg10d liquidity filtering, stricter wick/contact validation, improved anchor scoring, and chart commands that preload wedge lines. A manually drawn upper/lower pair can be saved specifically for future wedge scans; while valid, its exact anchors and projections take precedence over automatic detection and appear as **saved by me** only in the Kliny workflow. This dedicated save does not add wedge lines or saved status to Fibo, Ichimoku, or other chart contexts.
- **Trójpolówki (3P) watchlists** generated from allsearch output, with four active Fibo lifecycle columns plus a hidden-by-default 10-day dropout column and per-card Fibo dropout analysis, compact Ichimoku continuation/watch/cloud/retest columns, market ordering, freshness-sorted direction-marked top choices, per-cell market/scanner/direction metadata for filtering, consistent colored direction arrows, and persistent favorites with separate long/short 3P cards plus an always-visible table for saved instruments absent from current scanner classifications. Allsearch shows up to five genuinely qualified top choices per category without padding sparse groups; highlighted cards and Kliny breakouts put the newest pattern/breakout first, with a fresh valid Fibo pattern displacing the oldest listed choice when the group is full. The reports also retain information-slider preferences, checked-instrument lists, per-column `📊` StockHelper bulk-open buttons, linkable Stooq/chart controls, persistent instrument highlights, and PDF/PNG export controls.
...
- **Interactive chart tool** powered by TradingView Lightweight Charts, with a grouped toolbar, collapsible sidebar cards, an in-chart searchable instrument switcher for every symbol already available in the local CSV cache, shared saved PLN balance, synchronized favorites, scanner-derived automatic stops and liquidity-aware maximum-capital guidance, manual level selection, two-anchor percentage-difference measurement, optional Ichimoku overlay, optional Fibonacci/wedge lines, shaded and draggable saved Fibo anchors, persistent alternating palettes for multiple Fibo groups, viewport-stable two-click Fibo previews with 23.6% and full future-area projection, scanner-aware Fibo/wedge reset, two-step manual wedge drawing and wedge-only saving, alternate-wedge cycling controls, one-click PNG download, stock-CFD mode, clear-active-value controls, saved sessions, generated configs, chart snapshots, and a transaction-journal panel. The chart also provides live Fibo/wedge/sidetrend correction tools, authoritative user-saved sidetrends, a movable debug dialog, and a persistently resizable report drawer. Scanner-loaded Ichimoku breakouts and Fibo/retest patterns are drawn as fixed candle-index highlights with an interactive legend and hover details; multi-candle patterns use one undivided outer box.
- **Setup information panel** in the chart UI for scanner-loaded setups: Ichimoku shows scanner breakout/retest context plus CSV candles from the scanner check window, Fibo shows anchor dates/values, the 61.8 pattern name and confirming date (including recovered legacy commodity patterns such as OIL/`CB.F` dark-cloud cover), and wedge/Kliny shows touch diagnostics plus CSV candles since the oldest wedge anchor.
...
For Ichimoku, retest patterns are tied to the current cloud side: above-cloud setups search bullish patterns against the cloud top/bottom zone, while below-cloud setups search bearish patterns against the cloud bottom/top zone. One continuous cloud visit counts only the pattern at its best local reaction extreme (lowest for long, highest for short); a close back on the trend side ends that cycle, and a later cloud visit begins a new retest. For Fibo, patterns are tied to the 61.8 retracement, at least one candle in the complete formation must touch 61.8, and the stored pattern date is always the final confirming candle. The chart Setup information panel prefers scanner-provided pattern metadata and can recover a recent dark-cloud-cover confirmation from legacy commodity report context.
...
- Completed month-scale ranges are checked only after final anchor selection and only from the first anchor through the first 61.8 touch (or the newest candle if 61.8 has not been touched). Saved user ranges replace overlapping scanner-only ranges in chart review, while an unfinished range extending to the newest candle does not by itself invalidate a live impulse.
- Regular short formations reuse the long detector on vertically mirrored OHLC data for forex, commodities, DAX40, and US100. The 3P steep-impulse scan also evaluates short stock moves directly, including WIG instruments, and applies short-specific top, lower-low, and month-range validation.
- A selected second anchor cannot remain at a marginal high or low inside a completed month-scale sideways phase. The scanner either moves to the newest confirmed structural extreme or rejects the formation when the range has replaced the original impulse.
- If price exceeds the current impulse extreme, the scanner moves the top/bottom anchor to the new high/low and keeps the still-live formation in the strong-impulse column. A correction that reclaims 23.6 before touching 61.8 also returns to the strong-impulse column; if it crosses 23.6 again, it returns to the waiting column.
- After a confirmed 61.8 reversal pattern, later candle extremes—not only closes—are checked against the pattern stop. Any wick trading through that stop invalidates the formation.
- The near-61.8 watch state represents a pullback that is at least 75% of the way from 50% toward 61.8 (or deeper).
- Pattern evaluation begins around the **first** candle that touches 61.8. A valid connected formation may touch the level with its first, middle, or final candle; the reported `Pattern date` is the formation's final confirming candle. A later standalone hammer/shooting star is not accepted merely because an earlier unrelated candle touched the level.
...
- A user-saved Fibo that no longer validates is retained for a five-day grace period. Its save indicator turns red and shows the remaining time; if the same geometry becomes valid again the warning clears, otherwise its saved status and geometry are removed after the deadline.
...
| `STOCKHELPER_COMMODITIES_WORKERS` | `3` | Worker count for bounded parallel commodity Stooq web scans. Allsearch defaults Forex and commodities to `3`; other markets use all available CPU workers. |
...
- The line tool can draw either an ordinary line or a two-line wedge. Wedge anchors snap to candle extremes, projected endpoints can be moved into the future, and **Save wedge** stores only that pair for later Kliny scans. It does not save the rest of the chart or expose the wedge in other scanner techniques; use **Save chart** when all current chart changes should be persisted together.
- Debug tools provide live correction views for scanner Fibo anchors, wedge anchors, and month-scale sidetrends. Reports retain selected sidetrends and line overlays while open, include the instrument and complete relevant candle data, and update as chart handles move.
...
- The chart toolbar includes **⬇ PNG** for downloading the currently visible chart directly from the browser. While the position-calculation drawer is open, the downloaded PNG also includes the complete position-sizing table; close the drawer to export the chart without that table. **Reset Fibo** or **Reset scanner** restores the original scanner-drawn structure.
