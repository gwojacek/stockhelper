...
- **Trójpolówki (3P) watchlists** generated from allsearch output, with four active Fibo lifecycle columns plus a hidden-by-default 10-day dropout column and per-card Fibo dropout analysis, compact Ichimoku continuation/watch/cloud/retest columns, market ordering, direction-marked top choices, per-cell market/scanner/direction metadata for filtering, consistent colored direction arrows, persistent deduplicated favorites (including a final table for saved instruments absent from current scanner classifications) and information-slider preferences, checked-instrument lists, per-column `📊` StockHelper bulk-open buttons, linkable Stooq/chart controls, and PDF export from every report tab.
