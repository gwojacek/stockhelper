...
- scan market groups for Ichimoku cloud, Fibonacci setups, and falling-wedge (Kliny) formations with age-aware breakout/retest handling, a five-day invalid-save warning lifecycle, wick-based stop validation, structural Fibo re-anchoring around monthly ranges, improved 5.0 wedge anchor scoring, and alternate-wedge review;
...
- **Falling-wedge (Kliny) scanner** exported from the Fibo scan flow, including unbroken wedges and fresh breakouts (up to 5 candles after breakout), automatic structures capped at 30 candles, Avg10d liquidity filtering, stricter wick/contact validation, improved anchor scoring, and chart commands that preload wedge lines. Saved, provider-qualified chart sessions take precedence over automatic anchors, so scans use the wedge the user can see, including its projected boundaries, touch state, and persisted breakout state.
...
On the first Funding Choices appearance, StockHelper gives its JavaScript two
seconds to attach the consent handler before clicking. It then checks for and
