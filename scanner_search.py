...
WIG_SEARCH_TICKERS = [
    ...
    "MRC","ALI","TOR","PWX","BCM","CLC","DGA","MLG","MZA","PCR","IFR","EQU","SNX","UNT","UNF","YAN","ZRE","SKH","VGO",
    ...
    "MLS","ULG","CRJ","PAS","MQR","MOV","4MS","ICE","BBT","SLV","DBE","GOP","SIM","SPR","GIF","ALE","DAD","PCF","ANR","HUG",
...
def _repeated_range_acceleration_launch_long(
    ...
    for idx in range(search_left, search_right + 1):
        ...
        if not math.isfinite(prior_high) or not math.isfinite(confirmation_high):
            continue
        launch_expansion = (confirmation_high - candidate_low) / max(abs(candidate_low), 1e-9)
...
def run_fibo_search(target: str) -> int:
    ...
    _write_md_table(out_md,"WYNIKI FIBO #0 (3P steep incline / decline)",["Ticker","Dir","Status","Incline","Ratio(d)","Near61.8","Avg10d PLN","Saved by user","Link","Python command","Latest data?","Latest date","Expected date"],rows0_md)
