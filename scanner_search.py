...
ICHIMOKU_OUTPUT_DIR = SEARCH_OUTPUT_DIR / "ichimoku"
FIBO_OUTPUT_DIR = SEARCH_OUTPUT_DIR / "fibo"
...
def _prune_search_history(group_name: str, keep_last: int = 3) -> None:
    ...
    files = [p for p in ICHIMOKU_OUTPUT_DIR.glob(f"{base}*.csv") if p.is_file()]
...
def _print_results_with_links(results: list[ScanResult]) -> list[str]:
    ...
    if not results:
        print("Brak wyników.")
        return []
    print(f"{'Ticker':<10} {'Pozycja':<8} {'Świece':<8} {'Mies.':<6} {'Start':<12} {'Close':>10} {'Avg10d PLN':>14} {'Low<Th20':>10} {'Link':<0} {'Stockhelper':<0}")
    print("-" * 140)
...
def _print_flip_results_with_links(flip_results: list[FlipResult]) -> list[str]:
    ...
    print(f"{base_header} {event_headers} {'Link':<0} {'Stockhelper':<0}".rstrip())
    print("-" * 150)
    links: list[str] = []
...
def run_ichimoku_search(target: str) -> int:
    ...
    if group_name == "WIG":
        ...
        ICHIMOKU_OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
        out_csv = ICHIMOKU_OUTPUT_DIR / f"search_{group_name.lower()}_{datetime.now(UTC).strftime('%Y%m%d_%H%M%S')}.csv"
        ...
        md_rows = [[row.ticker, row.side, f"{row.respect_months:.1f}", f"[stooq]({_stooq_chart_url(row.ticker)})", f"`{_stockhelper_chart_cmd(row.ticker, show_ichimoku=True)}`"] for row in sorted(results, key=lambda r: r.respect_days, reverse=True)]
        _write_simple_md_report(out_csv.with_suffix(".md"), f"Ichimoku {group_name} WYNIKI 1", ["Ticker", "Side", "Months", "Stooq", "Stockhelper"], md_rows)
        ...
        out_csv_flip = ICHIMOKU_OUTPUT_DIR / f"search_{group_name.lower()}_{datetime.now(UTC).strftime('%Y%m%d')}_flips.csv"
        with out_csv_flip.open("w", newline="", encoding="utf-8") as fh:
            ...
            writer.writerow(["ticker", "previous_side", "current_side", "flip_date", "months_since_flip", "latest_retest_status", "retest_depth", "valid_retests_count", "first_valid_retest_pattern_date", *dynamic_cols, "stooq_link", "stockhelper_link"])
            for row in sorted(flip_results, key=lambda r: r.months_since_flip, reverse=True):
        ...
        md_rows_flip = [[row.ticker, row.previous_side, row.current_side, row.flip_date, f"{row.months_since_flip:.1f}", f"[stooq]({_stooq_chart_url(row.ticker)})", f"`{_stockhelper_chart_cmd(row.ticker, show_ichimoku=True)}`"] for row in sorted(flip_results, key=lambda r: r.months_since_flip, reverse=True)]
        _write_simple_md_report(out_csv_flip.with_suffix(".md"), f"Ichimoku {group_name} WYNIKI 2", ["Ticker", "Prev", "Current", "Flip date", "Months", "Stooq", "Stockhelper"], md_rows_flip)
...
def _print_fibo_results(
    ...
    if not rows1:
        ...
        print(f"{'Ticker':<10} {'Dir':<6} {'Status':<30} {'Pattern':<22} {'Incline':<23} {'Ratio(d)':>16} {'Touched_61.8_date':<16} {'Avg10Turn':>12} {'Near61.8':>10} {'Link':<0} {'Stockhelper':<0}")
        print("-" * 185)
        links = []
    ...
    if not rows2:
        print("Brak wyników.")
        return links
    print(f"{'Ticker':<10} {'Dir':<6} {'Pattern':<22} {'Incline':<23} {'Ratio(d)':>16} {'Touched_61.8_date':<16} {'Link':<0} {'Stockhelper':<0}")
    print("-" * 128)
    for r in rows2:
        link = _stooq_chart_url(r.ticker)
...
def _write_simple_md_report(path: Path, title: str, headers: list[str], rows: list[list[str]]) -> None:
...
def run_fibo_search(target: str) -> int:
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as ex:
        ...
        for fut in as_completed(fut_map):
            ...
            rows.extend(found)
    FIBO_OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    out_csv = FIBO_OUTPUT_DIR / f"fibo_search_{group_name.lower()}_{datetime.now(UTC).strftime('%Y%m%d_%H%M%S')}.csv"
    with out_csv.open("w", newline="", encoding="utf-8") as fh:
        w = csv.writer(fh)
    ...
    for row in rows1 + rows2:
        md_rows_fibo.append([
            ...
            row.direction,
            row.status,
            f"{row.incline_start_date}->{row.incline_end_date}",
            f"[stooq]({_stooq_chart_url(row.ticker)})",
            f"`{_stockhelper_chart_cmd(row.ticker, fibo=row)}`",
        ])
    _write_simple_md_report(
        out_csv.with_suffix(".md"),
        f"Fibo {group_name} results",
        ["Ticker", "Dir", "Status", "Incline", "Stooq", "Stockhelper"],
        md_rows_fibo,
    )
