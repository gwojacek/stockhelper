...
- Updated README usage to make Docker + `stock` the easiest path, including a quick command table, click-to-run `python run ...` snippets that redirect to the repo-local Docker `stock` wrapper on the host, update workflow after `git pull`, report/browser behavior, permissions, cleanup, and plain Docker alternatives.
