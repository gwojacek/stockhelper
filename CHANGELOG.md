...
- Updated README usage to make Docker + `stock` the easiest path, including a quick command table, click-to-run `make ...` and `./stock ...` shell snippets backed by a repo-local Docker wrapper, update workflow after `git pull`, report/browser behavior, permissions, cleanup, and plain Docker alternatives.
