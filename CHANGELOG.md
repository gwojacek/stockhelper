...
- Updated README usage to make Docker + `stock` the easiest path, including a quick command table, click-to-run `./stock` command snippets and a repo-local `stock` launcher, update workflow after `git pull`, report/browser behavior, permissions, cleanup, and plain Docker alternatives.
