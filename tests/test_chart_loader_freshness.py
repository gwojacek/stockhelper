...
def test_literal_commodity_falls_back_to_cached_history_plus_yahoo_when_stooq_is_blocked(monkeypatch, tmp_path):
    csv_path = tmp_path / "CC_F.csv"
    _df("2026-06-08").to_csv(csv_path, index=False)
    monkeypatch.setattr(loader, "local_csv_path_for_symbol", lambda *_args: csv_path)
    ...
    monkeypatch.setattr(
        ...
        lambda base, *_args, **_kwargs: (_df("2026-06-08", "2026-06-09"), "CC=F", None, 1),
    )

    df, source, source_symbol, _name, reason = loader._download_remote(
        symbol="COCOA", instrument_type="commodity", api_key=None, data_source="auto"
    )
...
def test_allsearch_ichimoku_probe_bounds_cache_ahead_search(monkeypatch, tmp_path, capsys):
    import os
    import scanner_search as scanner

    members = [f"W{i:02d}" for i in range(15)]
    cached = _df("2026-08-17", "2026-08-18")
    ...
    assert os.environ.get("STOCKHELPER_CACHE_ONLY") == "1"
    assert calls == members[:12]
    output = capsys.readouterr().out
    assert output.count("cache is newer, trying another") == 12
    assert "keeping the newer cache" in output


def test_allsearch_cache_ahead_probes_still_detect_midday_yahoo_update(monkeypatch, tmp_path):
    import scanner_search as scanner

    members = ["OLD1", "OLD2", "OLD3", "LIVE", "EXTRA"]
    cached = _df("2026-08-17", "2026-08-18")
    older = _df("2026-08-17")
    changed = cached.copy()
    ...
    def yahoo(symbol, *_args, **_kwargs):
        calls.append(symbol)
        return (changed.copy() if symbol == "LIVE" else older.copy(), symbol, None)

    monkeypatch.setattr(scanner, "_yahoo_download_window", yahoo)

    assert scanner._should_refresh_group_data("WIG", members, ".WA") is True
    assert calls == members[:4]
