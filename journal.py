...
def _row(entry: dict[str, Any], number: int = 1) -> str:
    ...
    review_inner = (
        ...
        f"<div class='section-title'>Review notes</div><textarea class='notes' rows='4'>{e(entry.get('review_notes'))}</textarea>"
...
async function captureCloseScreenshot(card,payload){{const img=card?.querySelector('.screens .thumb');if(!img)return '';try{{await img.decode?.();const canvas=document.createElement('canvas');canvas.width=img.naturalWidth||img.width;canvas.height=img.naturalHeight||img.height;const ctx=canvas.getContext('2d');ctx.drawImage(img,0,0,canvas.width,canvas.height);const price=Number(String(payload.exit_price||'').replace(',','.'));const hi=Number(String(card?.dataset.high||'').replace(',','.'));const lo=Number(String(card?.dataset.low||'').replace(',','.'));let y=Math.round(canvas.height*.50);if(Number.isFinite(price)&&Number.isFinite(hi)&&Number.isFinite(lo)&&hi!==lo){{y=Math.max(12,Math.min(canvas.height-12,Math.round(((hi-price)/(hi-lo))*canvas.height)));}}const label='💰 SOLD '+(payload.outcome||'')+' @ '+(payload.exit_price||'--');ctx.fillStyle='rgba(2,6,23,.82)';ctx.fillRect(0,0,Math.min(canvas.width,590),60);ctx.fillStyle=payload.outcome==='loss'?'#fecaca':'#86efac';ctx.font='bold 28px Arial';ctx.fillText(label,18,39);ctx.strokeStyle=payload.outcome==='loss'?'#ef4444':'#22c55e';ctx.lineWidth=5;ctx.beginPath();ctx.moveTo(0,y);ctx.lineTo(canvas.width,y);ctx.stroke();ctx.fillStyle=ctx.strokeStyle;ctx.beginPath();ctx.arc(Math.min(canvas.width-70,canvas.width*.78),y,13,0,Math.PI*2);ctx.fill();ctx.fillStyle='#fff';ctx.font='bold 18px Arial';ctx.fillText('SOLD',Math.min(canvas.width-128,canvas.width*.78+20),y+6);return canvas.toDataURL('image/png');}}catch(e){{console.warn('close screenshot failed',e);return '';}}}}
...
Object.assign(window,{{updateJournalEntry,deleteJournalEntry,closeJournalEntry,toggleCompressed,bulkDelete}});
</script></body></html>"""
