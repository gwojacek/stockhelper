...
def _row(entry: dict[str, Any], number: int = 1) -> str:
    ...
    review_inner = (
        ...
        "<pre class='preview'></pre><button class='btn btn-outline' type='button' onclick='previewCloseLine(this)'>🎯 Preview sold line</button><button class='btn btn-outline' type='button' onclick='closeJournalEntry(this)'>✅ Close position</button>"
    )
    if ended:
    ...
    return "".join([
        f"<article class='journal-card {e(status)} {e(outcome)}' data-year='{e(_entry_year(entry))}' data-high='{e(entry.get('high'))}' data-low='{e(entry.get('low'))}' data-entry='{e(entry.get('entry'))}' data-direction='{e(entry.get('direction'))}'>",
...
document.addEventListener('input',e=>{{if(e.target.classList.contains('exit-price')){{const box=e.target.closest('.review');const card=e.target.closest('.journal-card');const out=box?.querySelector('.js-close-price');if(out)out.textContent=e.target.value||'--';const entry=Number(String(card?.querySelector('.summary-entry')?.value||card?.dataset.entry||'').replace(',','.'));const sold=Number(String(e.target.value||'').replace(',','.'));if(Number.isFinite(entry)&&Number.isFinite(sold)){{const dir=String(card?.dataset.direction||'long').toLowerCase();const diff=dir==='short'?entry-sold:sold-entry;const sel=box?.querySelector('.outcome');if(sel)sel.value=diff>=0?'profit':'loss';}}}}}});
...
Object.assign(window,{{updateJournalEntry,deleteJournalEntry,closeJournalEntry,previewCloseLine,toggleCompressed,bulkDelete}});
</script></body></html>"""
