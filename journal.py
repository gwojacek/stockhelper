...
def _row(entry: dict[str, Any], number: int = 1) -> str:
    ...
    review_inner = (
        ...
        "<pre class='preview'></pre><button class='btn btn-outline' type='button' onclick='openCloseChart(this)'>📈 Open chart to adjust close</button><button class='btn btn-outline' type='button' onclick='previewCloseLine(this)'>🎯 Preview sold line</button><button class='btn btn-outline' type='button' onclick='closeJournalEntry(this)'>✅ Close position</button>"
    )
    if ended:
...
function openCloseChart(btn){{const card=btn.closest('.journal-card');const symbol=(card?.dataset.symbol||'').replace(/\\.(WA|PL)$/i,'');if(!symbol){{toast('No symbol for chart');return;}}const cmd='python run -c '+symbol;fetch('/run-command?command='+encodeURIComponent(cmd)).then(()=>toast('Chart opened for close adjustment')).catch(()=>toast('Open chart failed'));}}
function previewCloseLine(btn){{const box=btn.closest('.review');const card=btn.closest('.journal-card');const exitPrice=box?.querySelector('.exit-price')?.value||'';const payload={{outcome:inferOutcome(card,exitPrice),exit_price:exitPrice}};captureCloseScreenshot(card,payload).then(url=>{{const img=card?.querySelector('.screens .thumb');if(img&&url)img.src=url;}});}}
...
Object.assign(window,{{updateJournalEntry,deleteJournalEntry,closeJournalEntry,previewCloseLine,openCloseChart,toggleCompressed,bulkDelete}});
</script></body></html>"""
