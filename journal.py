...
function openCloseChart(btn){{const card=btn.closest('.journal-card');const symbol=(card?.dataset.symbol||'').replace(/\\.(WA|PL)$/i,'');if(!symbol){{toast('No symbol for chart');return;}}const box=btn.closest('.review');const entryPrice=card?.querySelector('.summary-entry')?.value||card?.dataset.entry||'';const closePrice=box?.querySelector('.exit-price')?.value||'';const stopLoss=box?.querySelector('.review-stop-loss')?.value||'';const cmd='python run -c '+symbol+' --journal-close-mode --journal-entry-id '+(box?.dataset.id||'')+(entryPrice?' --journal-entry-price '+entryPrice:'')+(closePrice?' --journal-close-price '+closePrice:'')+(stopLoss?' --journal-stop-loss '+stopLoss:'');const params=new URLSearchParams({{command:cmd,journal_close:'1',entry_id:box?.dataset.id||'',entry_price:entryPrice,close_price:closePrice,stop_loss:stopLoss}});const url='/open-chart?'+params.toString();const popup=window.open('about:blank','_blank');if(popup){{popup.location=url;popup.focus();const timer=setInterval(()=>{{if(popup.closed){{clearInterval(timer);location.reload();}}}},1000);}}else{{window.open(url,'_blank');}}toast('Opening close-adjust chart...');}}
