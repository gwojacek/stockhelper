...
def _row(entry: dict[str, Any], number: int = 1) -> str:
    ...
    review_inner = (
        ...
        f"<div class='section-title'>Direction</div><select class='review-direction'><option value='long'{_selected(entry.get('direction'), 'long')}>↗ LONG</option><option value='short'{_selected(entry.get('direction'), 'short')}>↘ SHORT</option></select>"
