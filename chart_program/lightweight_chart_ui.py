...
<head>
  ...
  <style>
    * {{ box-sizing: border-box; }}
    ...
    #scanner-highlight-legend i {{ width:14px; height:10px; display:inline-block; border-radius:3px; border:1px solid rgba(255,255,255,.45); }}
    #scanner-highlight-tooltip {{ display:none; position:absolute; z-index:55; pointer-events:none; max-width:280px; padding:8px 10px; border:1px solid #475569; border-radius:8px; background:rgba(15,23,42,.94); color:#e5e7eb; font-size:12px; font-weight:800; box-shadow:0 10px 28px rgba(0,0,0,.35); }}
...
<body>
  ...
  <div class="layout">
    <main class="main">
      ...
      <section id="debug-dialog" role="dialog" aria-modal="false" aria-labelledby="debug-dialog-title"><div class="debug-head"><h3 id="debug-dialog-title">Debug tools</h3><button id="debug-dialog-close" class="debug-close" type="button" aria-label="Close">×</button></div><div id="debug-dialog-body"></div></section>
    </main>
    <aside class="side">
...
(() => {{
  ...
  const resetLegend = () => {{ $('chart-legend').innerHTML = ''; $('scanner-highlight-legend').innerHTML = ''; }};
  ...
  let debugSidetrendReportFilter = 'all';
  ...
  function showCorrectionReport(refreshOnly=false) {{
    ...
    const sidetrendTabs=mode==='sidetrend'?`<span class="debug-report-tabs" role="tablist"><button id="debug-report-all" type="button" role="tab" aria-selected="${{debugSidetrendReportFilter==='all'}}" class="${{debugSidetrendReportFilter==='all'?'active':''}}">All sidetrends</button><button id="debug-report-changed" type="button" role="tab" aria-selected="${{debugSidetrendReportFilter==='changed'}}" class="${{debugSidetrendReportFilter==='changed'?'active':''}}">Changed / invalid</button></span>`:'';
    $('calc-summary').innerHTML=`${{sidetrendTabs}}<button id="copy-debug-report" type="button">Copy result</button>${{canKeep?'<button id="keep-debug-correction" type="button">Save changed '+mode+'</button>':''}}`;
    ...
    const rows=[];
    const sidetrendChanged=r=>!!r.markedInvalid||r.scannerStart==='not found'||r.start!==r.scannerStart||r.end!==r.scannerEnd;
    const sidetrendCoverage=r=>{{
      const scannerFound=r.scannerStart!=='not found'&&r.scannerEnd!=='not found';
      return {{start:scannerFound&&r.scannerStart<r.start?r.scannerStart:r.start,end:scannerFound&&r.scannerEnd>r.end?r.scannerEnd:r.end}};
    }};
    const sidetrendDateDifference=r=>{{
      if(r.markedInvalid)return'Removed';
      if(r.scannerStart==='not found')return'Added';
      const delta=(before,after)=>Math.round((Date.parse(after)-Date.parse(before))/86400000);
      const startDelta=delta(r.scannerStart,r.start),endDelta=delta(r.scannerEnd,r.end);
      if(!startDelta&&!endDelta)return'—';
      const signed=value=>`${{value>0?'+':''}}${{value}}d`;
      return `Start ${{signed(startDelta)}}; End ${{signed(endDelta)}}`;
    }};
    const reportSidetrends=[...(debugSideRanges||[])].filter(r=>(r.valid||r.markedInvalid)&&(debugSidetrendReportFilter!=='changed'||sidetrendChanged(r))).sort((a,b)=>String(b.start).localeCompare(String(a.start)));
    if(mode==='fibo') {{
    ...
    else reportSidetrends.forEach(r=>{{
      const days=(start,end)=>Math.max(0,Math.round((Date.parse(end)-Date.parse(start))/86400000));
      const scannerFound=r.scannerStart!=='not found',modified=r.markedInvalid?'Marked invalid':(r.start!==r.scannerStart||r.end!==r.scannerEnd?'Adjusted':'No changes');
      rows.push([r.id,scannerFound?r.scannerStart:'—',scannerFound?r.scannerEnd:'—',scannerFound?String(days(r.scannerStart,r.scannerEnd)):'—',r.markedInvalid?'—':r.start,r.markedInvalid?'—':r.end,r.markedInvalid?'—':String(days(r.start,r.end)),sidetrendDateDifference(r),modified]);
    }});
    ...
    $('debug-report-all')?.addEventListener('click',()=>{{debugSidetrendReportFilter='all';showCorrectionReport(true);}});
    $('debug-report-changed')?.addEventListener('click',()=>{{debugSidetrendReportFilter='changed';showCorrectionReport(true);}});
  ...
  async function keepDebugCorrection() {{
    if (debugReportMode==='sidetrend') {{
      ...
      levels.__saved_invalid_sidetrends__=(debugSideRanges||[]).filter(r=>r.markedInvalid).map(r=>({{scannerStart:r.scannerStart,scannerEnd:r.scannerEnd,start:r.start,end:r.end}}));
      ...
      if(resp.ok&&data.ok) levels={{...levels,__saved_sidetrends__:payload.__saved_sidetrends__,__saved_invalid_sidetrends__:payload.__saved_invalid_sidetrends__}};
      return;
    }}
  ...
  function renderSidetrendEditor() {{
    ...
    debugSideRanges.forEach((range,index)=>{{
      if(!range.valid&&!range.markedInvalid&&!debugShowUnselected)return;
      const row=document.createElement('tr'); row.className=range.valid?'':(range.markedInvalid?'marked-invalid':'invalid');
      const color=sidetrendColor(index);
      const days=Math.max(0,Math.round((Date.parse(range.end)-Date.parse(range.start))/86400000));
      row.innerHTML=`<td><input type="checkbox" ${{range.valid?'checked':''}} aria-label="Keep ${{range.id}}"></td><td><i style="display:inline-block;width:9px;height:9px;margin-right:4px;border-radius:2px;background:${{color.stroke}}"></i>${{range.id}}</td><td><input type="date" value="${{range.start}}" aria-label="${{range.id}} start"></td><td><input type="date" value="${{range.end}}" aria-label="${{range.id}} end"></td><td>${{days}}</td><td title="${{range.scannerStart==='not found'?'Added by user':'Detected by scanner'}}">${{range.markedInvalid?'Invalid':(range.scannerStart==='not found'?'—':'✓')}}</td><td><button type="button" class="mark-invalid">${{range.markedInvalid?'Restore':'Mark invalid'}}</button><button type="button" class="delete-range" aria-label="Delete ${{range.id}}">×</button></td>`;
      const [valid,start,end]=row.querySelectorAll('input');
      valid.onchange=()=>{{range.valid=valid.checked;if(range.valid)range.markedInvalid=false;renderSidetrendEditor();}};
      start.onchange=()=>{{range.start=start.value; drawCloud();}}; end.onchange=()=>{{range.end=end.value; drawCloud();}};
      row.querySelector('.mark-invalid').onclick=()=>{{range.markedInvalid=!range.markedInvalid;range.valid=!range.markedInvalid;renderSidetrendEditor();}};
      row.querySelector('.delete-range').onclick=()=>{{debugSideRanges.splice(index,1);renderSidetrendEditor();}};
    ...
    $('debug-toggle-all').onclick=()=>{{const select=!debugSideRanges.every(r=>r.valid);debugSideRanges.forEach(r=>{{r.valid=select;if(select)r.markedInvalid=false;}});renderSidetrendEditor();}};
  ...
  function renderDebugChooser() {{
    ...
    const body=$('debug-dialog-body');
    if (tech==='Fibo') body.innerHTML='<p class="debug-help">Choose what you want to review and correct.</p><div class="debug-choice-grid"><button class="debug-choice" id="choose-sidetrends"><strong>Sidetrend scanner</strong><span>Review all sideways periods longer than one month.</span></button><button class="debug-choice" id="choose-fibo-anchors"><strong>Fibo anchors</strong><span>Compare scanner anchors with your corrected anchors.</span></button></div>';
    else body.innerHTML='<div class="debug-choice-grid"><button class="debug-choice" id="choose-wedge-anchors"><strong>Wedge anchors</strong><span>Correct the scanner upper and lower wedge lines.</span></button></div>';
    $('choose-sidetrends')?.addEventListener('click',renderSidetrendEditor);
    $('choose-fibo-anchors')?.addEventListener('click',()=>renderGeometryEditor('fibo'));
    $('choose-wedge-anchors')?.addEventListener('click',()=>renderGeometryEditor('wedge'));
  ...
  function scannerPatternSpan(name) {{
    const n = String(name || '').toLowerCase().replace(/[-\\s]+/g, '_');
    if (/engulfing|piercing_line|dark_cloud_cover|harami/.test(n)) return 2;
    if (/morning_star|evening_star|three_|3_/.test(n)) return 3;
    return 1;
  }}
  ...
  function scannerCandleBand(time, span=1) {{
    const row = ohlcByTime.get(String(time).slice(0, 10));
    if (!row) return null;
    const startIdx = Math.max(0, row.idx - Math.max(1, span) + 1);
    const endIdx = row.idx;
    const x0 = chart.timeScale().timeToCoordinate(ohlc[startIdx]?.time);
    const x1 = chart.timeScale().timeToCoordinate(ohlc[endIdx]?.time);
    if (!Number.isFinite(x0) || !Number.isFinite(x1)) return null;
    const prevX = startIdx > 0 ? chart.timeScale().timeToCoordinate(ohlc[startIdx - 1].time) : null;
  ...
  function addScannerHighlightLegend(events) {{
    ...
    events.forEach(ev => {{
      ...
      item.classList.toggle('hidden', hiddenLegendKeys.has(ev.key));
      item.innerHTML = `<i style="background:${{ev.color}}"></i><b>${{ev.label}}</b>`;
      item.onclick = () => {{ hiddenLegendKeys.has(ev.key) ? hiddenLegendKeys.delete(ev.key) : hiddenLegendKeys.add(ev.key); render(); }};
      legend.appendChild(item);
    }});
  }}
  ...
  function drawScannerHighlights(ctx) {{
    ...
    const events = scannerHighlightEvents();
    addScannerHighlightLegend(events);
    events.forEach(ev => {{
  ...
  function render() {{
    ...
    drawnObjects.forEach(obj => {{
      ...
      if (isWedge) {{
        ...
        series = addLine([{{time:String(obj.x0).slice(0,10), value:Number(obj.y0)}}, {{time:x1, value:Number(obj.y1)}}], color, isFib && String(obj.label || '').includes('61.8%') ? 1.4 : (isFib ? 1.0 : 2), LightweightCharts.LineStyle.Solid, seriesTitle, showLegend && !isFib, false, isFib, objKey, deleteFn, !isEditableLineObject(obj) && !isFib && !isFibBoundary);
      }}
  ...
  chart.subscribeClick(param => {{
    ...
    if (activeTool === 'sidetrend-add') {{
      const date=nearest(time).time;
      if (!newSidetrendAnchor) {{newSidetrendAnchor=date;$('debug-add-sidetrend').textContent='Click second candle…';return;}}
      const start=newSidetrendAnchor<date?newSidetrendAnchor:date,end=newSidetrendAnchor<date?date:newSidetrendAnchor;
      debugSideRanges.push({{id:`S${{debugSideRanges.length+1}}`,scannerStart:'not found',scannerEnd:'not found',start,end,valid:true}});
      newSidetrendAnchor=null; activeTool='level'; renderSidetrendEditor(); drawCloud(); return;
    }}
