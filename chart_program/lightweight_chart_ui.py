...
(() => {{
  ...
  function objectHit(obj, pt) {{
    ...
    const wedgeEnd = isWedge ? wedgeProjectedEnd(obj) : null;
    const xs = isWedge && Array.isArray(obj.anchor_x) && obj.anchor_x.length >= 1 && wedgeEnd ? [obj.anchor_x[0], wedgeEnd.time] : (Array.isArray(obj.x) ? obj.x : [obj.x0, obj.x1]);
    const ys = isWedge && Array.isArray(obj.anchor_y) && obj.anchor_y.length >= 1 && wedgeEnd ? [obj.anchor_y[0], wedgeEnd.value] : (Array.isArray(obj.y) ? obj.y : [obj.y0, obj.y1]);
  ...
  function editObject(obj, from, to, mode='move') {{
    ...
    const rebuildWedgeProjection = () => {{
      ...
      const end = wedgeProjectedEnd(obj);
      if (!end) return;
      obj.x = [x0, end.time]; obj.y = [roundPrice(y0), end.value];
      obj.x0 = x0; obj.y0 = roundPrice(y0); obj.x1 = end.time; obj.y1 = end.value; obj.price = end.value;
    }};
    const setEndpoint = (idx) => {{
      ...
      if (isWedge && idx > 0) {{
        ...
        obj.display_x1 = editTime; obj.display_y1 = editPrice; rebuildWedgeProjection(); return;
      }}
    ...
    if (obj.display_x1) obj.display_x1 = shiftDate(obj.display_x1);
    if (Number.isFinite(Number(obj.display_y1))) obj.display_y1 = shiftPrice(obj.display_y1);
    rebuildWedgeProjection();
  }}
  ...
  function lineDataForObject(obj) {{
    if ((obj.type === 'wedge' || obj.group_id === 'auto-wedge') && Array.isArray(obj.anchor_x) && Array.isArray(obj.anchor_y) && obj.anchor_x.length >= 2 && obj.anchor_y.length >= 2) {{
      ...
      const y0 = Number(obj.anchor_y[0]), y1 = Number(obj.anchor_y[obj.anchor_y.length - 1]);
      const end = wedgeProjectedEnd(obj);
      if (end && Number.isFinite(y0)) return [{{time:x0, value:roundPrice(y0)}}, {{time:end.time, value:end.value}}];
    }}
  ...
  function wedgeProjectedEnd(obj) {{
    if (!(obj?.type === 'wedge' || obj?.group_id === 'auto-wedge') || !Array.isArray(obj.anchor_x) || !Array.isArray(obj.anchor_y) || obj.anchor_x.length < 2 || obj.anchor_y.length < 2) return null;
    const x0 = String(obj.anchor_x[0]).slice(0,10), x1 = String(obj.anchor_x[obj.anchor_x.length - 1]).slice(0,10);
    const y0 = Number(obj.anchor_y[0]), y1 = Number(obj.anchor_y[obj.anchor_y.length - 1]);
    const t0 = new Date(x0 + 'T00:00:00Z').getTime(), t1 = new Date(x1 + 'T00:00:00Z').getTime();
    if (!Number.isFinite(t0) || !Number.isFinite(t1) || !Number.isFinite(y0) || !Number.isFinite(y1)) return null;
    const maxTime = addDays(P.ohlc[P.ohlc.length - 1]?.time || x1, 90);
    let endTime = String(obj.display_x1 || maxTime).slice(0,10);
    if (compareTime(endTime, maxTime) > 0) endTime = maxTime;
    const explicitY = Number(obj.display_y1);
    if (Number.isFinite(explicitY)) return {{time:endTime, value:roundPrice(explicitY)}};
    const spanDays = Math.max(1, Math.round((t1 - t0) / 86400000));
    const endDays = Math.max(1, Math.round((new Date(endTime + 'T00:00:00Z').getTime() - t0) / 86400000));
    return {{time:endTime, value:roundPrice(y0 + (y1 - y0) * (endDays / spanDays))}};
  }}
