...
- `STOCKHELPER_STOOQ_DEBUG_HTTP=1` saves direct/cloudscraper/playwright-stealth Stooq response diagnostics under `data/debug/stooq_http/` (URL with API key redacted, status, headers, anti-bot detection flag/kind, preview, and response body excerpt). The console error also preserves the debug JSON path.
